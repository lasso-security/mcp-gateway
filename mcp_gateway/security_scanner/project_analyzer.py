...
from mcp_gateway.security_scanner.config import Keys, PERMISSIVE_LICENSES, MarketPlaces, logger, formatter
...
class ProjectAnalyzer:
    ...
    def get_project_data(self, market_place: Optional[str], project_name: str) -> Dict[str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            github_fetcher = GithubFetcher(project_metadata.get(Keys.GITHUB_LINK))
            github_repo_metadata = github_fetcher.get_repository_metadata()
            github_owner_metadata = github_fetcher.get_owner_metadata()
            ...
            logging.error(f"Error collecting data for {project_name}: {e}", exc_info=True)
            raise
    ...
    def _score_repo_age(self, created_at_str: Union[str, None]) -> float:
        ...
        try:
            ...
            logging.warning(f"Could not parse repository created_at date: {created_at_str}")
            return 0.0
    def _calculate_owner_score(self, owner_data: Dict[str, Any]) -> float:
        ...
        followers_score = self._normalize_log(owner_data.get(Keys.FOLLOWERS, 0), MAX_LOG_FOLLOWERS)
        repos_score = self._normalize_log(owner_data.get(Keys.PUBLIC_REPOS_NUMBER, 0), MAX_LOG_REPOS)
        ...
        org_bonus = 0.3 if owner_data.get(Keys.OWNER_TYPE) == Keys.ORGANIZATION_OWNER_TYPE else 0.0
        ...
        score = min(score, 1.0)
        logging.info(f"Owner Score details: Value: {score:.2f} (Followers: {followers_score:.2f}, Repos: {repos_score:.2f}, Age: {account_age_score:.2f}, Verified: {verified_bonus:.2f}, Org: {org_bonus:.2f}, Blog: {blog_bonus:.2f})")
    ...
    def _calculate_repo_score(self, repo_data: Dict[str, Any]) -> float:
        ...
        # Ensure score is capped at 1.0 before applying main weight
        score = min(score, 1.0)
        logger.info(f"Repo Score: {score:.2f} (Stars: {stars_score:.2f}, Forks: {forks_score:.2f}, License: {license_score:.2f}, Age: {repo_age_score:.2f})")
        return score * 100 # Scale to 0-100
    ...
    def _calculate_smithery_score(self, project_data: Dict[str, Any]) -> float:
        ...
        tool_calls_score = self._normalize_log(project_data.get(Keys.MONTHLY_TOOL_USAGE, 0), MAX_LOG_TOOL_CALLS)
        ...
        score = (
         ...
         # Ensure score is capped at 1.0 before applying main weight
        score = min(score, 1.0)
        logging.debug(f"Smithery Score: {score:.2f} (ToolCalls: {tool_calls_score:.2f}, Verified: {verified_bonus:.2f}, LicenseMatch: {license_bonus:.2f})")
        return score * 100 # Scale to 0-100

    def _calculate_npm_score(self, project_data: Dict[str, Any]) -> float:
        """Calculates the reputation score for the npm project listing."""
        if not project_data or project_data.get(Keys.ERROR): # MODIFIED: Used Keys.ERROR
            logging.warning("NPM project data is missing or contains an error, assigning zero score.")
            return 0.0

        package_name = project_data.get(Keys.PACKAGE_NAME, 'Unknown Package') # MODIFIED
        ...
        if isinstance(license_val, str):
            ...
            license_key_to_score = license_val.get("type") # Common field for SPDX type in npm license objects
        
        license_score_val = self._score_license(license_key_to_score)

        maintainers_score = self._normalize_log(num_maintainers or 0, MAX_LOG_NPM_MAINTAINERS)
        ...
        final_npm_score = min(score, 1.0) * 100 # Ensure score is capped at 1.0 before scaling to 0-100
        logger.info(f"Calculated NPM Score for {package_name}: {final_npm_score:.2f}/100")
    ...
    def _score_npm_recency(self, days_since_updated: Optional[int]) -> float:
        ...
        if days_since_updated is None or days_since_updated < 0:
            logging.debug("NPM recency: No valid 'days_since_updated' data, scoring 0.0.")
            return 0.0 
        if days_since_updated <= 30: # Updated within the last month
            ...
            logging.debug(f"NPM recency: Package not updated for {days_since_updated} days, scoring 0.0.")
            return 0.0
    ...
    def analyze(self, market_place: Optional[str], project_name: str) -> Tuple[float, Dict[str, float]]:
        """
        ...
        """
        try:
            project_data = self.get_project_data(market_place=market_place, project_name=project_name)
            ...
            if market_place == MarketPlaces.SMITHERY:
                ...
                project_score = repo_score * NPM_REPO_WEIGHT_IN_PACKAGE_SCORE + source_score * NPM_REGISTRY_WEIGHT_IN_PACKAGE_SCORE
            ...
            logger.info(f"Reputation analysis complete for {project_name}. Final Score: {final_score:.2f}")
            logger.info(f"Component Scores: {component_scores}")

            return round(final_score, 2), component_scores

        except Exception as e:
            logging.error(f"Error during analysis for {project_name}: {e}", exc_info=True)
            # Return a default score or re-raise depending on desired behavior
            return 0.0, {Keys.OWNER_SCORE: 0.0, Keys.REPO_SCORE: 0.0, Keys.SMITHERY_SCORE: 0.0}
...
if __name__ == '__main__':
    ...
    try:
        ...
        test = analyzer.get_project_data(market_place='smithery', project_name='@smithery-ai/server-sequential-thinking')
        print(test)
        final_score, component_scores = analyzer.analyze(market_place='smithery', project_name='@smithery-ai/server-sequential-thinking')
